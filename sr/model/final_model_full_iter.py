...
class Top_Down_Baseline(nn.Module):
    def __init__(self, convnet, role_emb, verb_emb, out_init, query_composer, v_att, q_net, v_net,
                 neighbour_attention, updated_query_composer, Dropout_C, classifier, encoder, ans_combiner):
        ...
        #self.se_img = se_img
        #self.se_ans = se_ans
        self.ans_combiner = ans_combiner
...
def build_top_down_query_context_only_baseline(n_roles, n_verbs, num_ans_classes, encoder):
    ...
    out_init = nn.Parameter(torch.zeros( encoder.max_role_count, hidden_size))
    ...
    ans_combiner = nn.Linear(hidden_size*2, hidden_size)

    #se_img = SELayer(img_embedding_size)
    #se_ans = SELayer(hidden_size, reduction=8)

    return Top_Down_Baseline(covnet, role_emb, verb_emb, out_init, query_composer, v_att, q_net,
                             v_net, neighbour_attention, updated_query_composer, Dropout_C, classifier, encoder,  ans_combiner)

...
class Top_Down_Baseline(nn.Module):
    def __init__(self, covnet, role_emb, verb_emb, query_composer, v_att, q_net,
                 v_net, neighbour_attention, v_att_ctx, q_net_ctx, v_net_ctx, Dropout_C,
    ...
    def forward(self, v_org, gt_verb):
        ...
        updated_q_emb = self.Dropout_C(withctx)

        att = self.v_att_ctx(img, updated_q_emb)
        v_emb = (att * img).sum(1)
        v_repr = self.v_net_ctx(v_emb)
        q_repr = self.q_net_ctx(updated_q_emb)
        ...
        print(baseline_confidence_norm, qctx_confidence_norm)
...
def build_top_down_query_context_only_baseline(n_roles, n_verbs, num_ans_classes, encoder):
    ...
    #sep ctx
    v_att_ctx = Attention(img_embedding_size, hidden_size, hidden_size)
    q_net_ctx = FCNet([hidden_size, hidden_size ])
    v_net_ctx = FCNet([img_embedding_size, hidden_size])

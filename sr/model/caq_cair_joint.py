...
class Top_Down_Baseline(nn.Module):
    def __init__(self, caq_model, flatten_img, reconstruct_img, encoder):
    ...
    def forward(self, v_org, gt_verb):
        ...
        cur_group = caq_hidden_rep.contiguous().view(v_org.size(0), -1)
        constructed_img = self.reconstruct_img(cur_group)

        logits = self.caq_model.classifier(caq_hidden_rep)
...
def build_caq_cai_joint(caq_model, encoder):
    ...
    return Top_Down_Baseline(caq_model, flatten_img, reconstruct_img, encoder)
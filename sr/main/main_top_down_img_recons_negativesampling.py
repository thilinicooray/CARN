...
from sr.model import top_down_img_recons_negativesampling
...
def train(model, train_loader, dev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, model_name, model_saving_name, eval_frequency=4000):
    ...
    for epoch in range(max_epoch):
        ...
        for i, (_, img, verb, labels, negative_samples) in enumerate(train_loader):
            total_steps += 1
            ...
            if gpu_mode >= 0:
                ...
                negative_samples = torch.autograd.Variable(negative_samples.cuda())
            else:
                img = torch.autograd.Variable(img)
                verb = torch.autograd.Variable(verb)
                labels = torch.autograd.Variable(labels)
                negative_samples = torch.autograd.Variable(negative_samples)

            role_predict, constructed_img, flattened_img, negative_img_embed = pmodel(img, verb, negative_samples)
            loss_entropy, loss_recons = model.calculate_loss(verb, role_predict, labels, constructed_img, flattened_img, negative_img_embed)
...
def eval(model, dev_loader, encoder, gpu_mode, write_to_file = False):
    ...
    with torch.no_grad():
        ...
        for i, (img_id, img, verb, labels, _) in enumerate(dev_loader):
            ...
            role_predict = model(img, verb, None)
...
def main():
    ...
    train_set = imsitu_loader.imsitu_loader_negative_sampling(imgset_folder, train_set, encoder,'train', encoder.train_transform)

    constructor = 'build_%s' % args.model
    model = getattr(top_down_img_recons_negativesampling, constructor)(encoder.get_num_roles(),encoder.get_num_verbs(), encoder.get_num_labels(), encoder)
    ...
    dev_set = imsitu_loader.imsitu_loader_negative_sampling(imgset_folder, dev_set, encoder, 'val', encoder.dev_transform)
    ...
    test_set = imsitu_loader.imsitu_loader_negative_sampling(imgset_folder, test_set, encoder, 'test', encoder.dev_transform)

...
from sr.model import td_feat_caq
...
def train(model, train_loader, dev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, model_name, model_saving_name, eval_frequency=4):
    ...
    for epoch in range(max_epoch):
        t = time.time()
        for i, (_, img, verb, labels, td_feat) in enumerate(train_loader):
            total_steps += 1
            min_t = time.time()
            if gpu_mode >= 0:
                ...
                td_feat = torch.autograd.Variable(td_feat.cuda())
            else:
                img = torch.autograd.Variable(img)
                verb = torch.autograd.Variable(verb)
                labels = torch.autograd.Variable(labels)
                td_feat = torch.autograd.Variable(td_feat)

            role_predict = pmodel(img, verb, td_feat)
...
def eval(model, dev_loader, encoder, gpu_mode, write_to_file = False):
    ...
    with torch.no_grad():
        t1 = time.time()
        for i, (img_id, img, verb, labels, td_feat) in enumerate(dev_loader):
            ...
            role_predict = model(img, verb, td_feat)
...
def main():
    ...
    train_set = imsitu_loader.imsitu_loader_tdfeat(imgset_folder, train_set, encoder,'train', encoder.train_transform)


    constructor = 'build_%s' % args.model
    model = getattr(td_feat_caq, constructor)(encoder.get_num_roles(),encoder.get_num_verbs(), encoder.get_num_labels(), encoder)
    ...
    dev_set = imsitu_loader.imsitu_loader_tdfeat(imgset_folder, dev_set, encoder, 'val', encoder.dev_transform)
    ...
    test_set = imsitu_loader.imsitu_loader_tdfeat(imgset_folder, test_set, encoder, 'test', encoder.dev_transform)

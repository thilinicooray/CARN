...
def train(model, train_loader, dev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, model_name, model_saving_name, eval_frequency=4000):
    ...
    for epoch in range(max_epoch):
        ...
        for i, (_, img, verb_img_feat, verb, labels) in enumerate(train_loader):
            total_steps += 1
            ...
            verb_predict, role_predict = pmodel(img, verb_img_feat, verb)
            ...
            top1.add_point_both(verb_predict, verb, role_predict, labels)
            top5.add_point_both(verb_predict, verb, role_predict, labels)
...
def eval(model, dev_loader, encoder, gpu_mode, write_to_file = False):
    ...
    with torch.no_grad():
        ...
        for i, (img_id, img, verb_img_feat, verb, labels) in enumerate(dev_loader):
            ...
            verb_predict, role_pred_topk = model.forward_eval(img, verb_img_feat)

            top1.add_point_eval5_log_sorted(img_id, verb_predict, verb, role_pred_topk, labels)
...
def main():
    ...
    model = getattr(ggnn_with_verb, constructor)(encoder.get_num_roles(),encoder.get_num_verbs(), encoder.get_num_labels(), encoder)
    ...
    dev_set = imsitu_loader.imsitu_loader_verbimgfeat_4_role(imgset_folder, dev_set, encoder, 'val', encoder.dev_transform)
    ...
    test_set = imsitu_loader.imsitu_loader_verbimgfeat_4_role(imgset_folder, test_set, encoder, 'test', encoder.dev_transform)

...
def train(model, train_loader, dev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, model_name, model_saving_name, eval_frequency=4):
    ...
    for epoch in range(max_epoch):
        ...
        for i, (img_id, img, verb, agent, place) in enumerate(train_loader):
            total_steps += 1

            if gpu_mode >= 0:
                img = torch.autograd.Variable(img.cuda())
                agent = torch.autograd.Variable(agent.cuda())
                place = torch.autograd.Variable(place.cuda())
                verb = torch.autograd.Variable(verb.cuda())
            else:
                img = torch.autograd.Variable(img)
                agent = torch.autograd.Variable(agent)
                place = torch.autograd.Variable(place)
                verb = torch.autograd.Variable(verb)

            verb_predict = pmodel(img, agent, place)
            ...
            del verb_predict, loss, img , agent, place, verb
...
def eval(model, dev_loader, encoder, gpu_mode, write_to_file = False):
    ...
    with torch.no_grad():
        ...
        for i, (img_id, img, verb, agent, place) in enumerate(dev_loader):
            ...
            verb_predict = model(img, agent, place)
...
def main():
    ...
    train_set = imsitu_loader.imsitu_loader_agentplace_4_verb(imgset_folder, train_set, encoder,'train', encoder.train_transform)

    #building current model
    constructor = 'build_%s' % args.model
    model = getattr(top_down_only_verb_gt, constructor)(len(encoder.agent_label_list), len(encoder.place_label_list),
    ...
    dev_set = imsitu_loader.imsitu_loader_agentplace_4_verb(imgset_folder, dev_set, encoder, 'val', encoder.dev_transform)
    ...
    test_set = imsitu_loader.imsitu_loader_agentplace_4_verb(imgset_folder, test_set, encoder, 'test', encoder.dev_transform)

...
from sr.model import verb_top_down_baseline_pretrained_img_feat
...
def train(model, train_loader, dev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, model_name, model_saving_name, eval_frequency=4000):
    ...
    for epoch in range(max_epoch):
        ...
        for i, (img_id, verb_grid_features, agent_flat_features, place_flat_features, verb) in enumerate(train_loader):
            total_steps += 1

            if gpu_mode >= 0:
                verb_grid_features = torch.autograd.Variable(verb_grid_features.cuda())
                ...
                verb_grid_features = torch.autograd.Variable(verb_grid_features)
            ...
            verb_predict = pmodel(verb_grid_features, agent_flat_features, place_flat_features)
            ...
            del verb_predict, loss, verb_grid_features, agent_flat_features, place_flat_features, verb
...
def eval(model, dev_loader, encoder, gpu_mode, write_to_file = False):
    ...
    with torch.no_grad():
        ...
        for i, (img_id, verb_grid_features, agent_flat_features, place_flat_features, verb) in enumerate(dev_loader):
            ...
            verb_predict = model(verb_grid_features, agent_flat_features, place_flat_features)

            top1.add_point_verb_only_eval(img_id, verb_predict, verb)
            top5.add_point_verb_only_eval(img_id, verb_predict, verb)

            del verb_predict, verb_grid_features, agent_flat_features, place_flat_features, verb
            #break
...
def main():
    ...
    train_set = imsitu_loader.imsitu_loader_verb_pretrained_img_feat(imgset_folder, train_set, encoder,'train', encoder.train_transform)

    constructor = 'build_%s' % args.model
    model = getattr(verb_top_down_baseline_pretrained_img_feat, constructor)(len(encoder.verb_list))
    ...
    dev_set = imsitu_loader.imsitu_loader_verb_pretrained_img_feat(imgset_folder, dev_set, encoder, 'val', encoder.dev_transform)
    ...
    test_set = imsitu_loader.imsitu_loader_verb_pretrained_img_feat(imgset_folder, test_set, encoder, 'test', encoder.dev_transform)

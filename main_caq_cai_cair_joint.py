...
from sr.model import caq_cai_cair_joint, top_down_query_context_pretrained_baseline, top_down_image_context_pretrained_baseline, top_down_baseline
...
def train(model, train_loader, dev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, model_name, model_saving_name, eval_frequency=4000):
    ...
    for epoch in range(max_epoch):
        ...
        for i, (_, img, verb, labels) in enumerate(train_loader):
            ...
            loss = model.calculate_loss(verb, role_predict, labels, constructed_img, flattened_img)
...
def main():
    ...
    parser.add_argument('--model', type=str, default='caq_cai_joint')
    ...
    parser.add_argument('--cai_model', type=str, default='', help='Pretrained cai model')
    ...
    caq_model = getattr(top_down_query_context_pretrained_baseline, constructor)(encoder.get_num_roles(),encoder.get_num_verbs(), encoder.get_num_labels(), encoder, baseline)

    constructor = 'build_top_down_query_context_only_baseline'
    cai_model = getattr(top_down_image_context_pretrained_baseline, constructor)(encoder.get_num_roles(),encoder.get_num_verbs(), encoder.get_num_labels(), encoder, baseline)

    constructor = 'build_%s' % args.model
    model = getattr(caq_cai_cair_joint, constructor)(caq_model, cai_model, encoder)
    ...
    if args.resume_training:
        ...
        utils.load_net(args.cai_model, [model.cai_model])
        utils.set_trainable(model.cai_model, False)
        utils.set_trainable(model.cai_model.q_net, True)
        utils.set_trainable(model.cai_model.v_net, True)
        optimizer = torch.optim.Adamax([
            ...
            {'params': model.cai_model.q_net.parameters(), 'lr': 5e-5},
            {'params': model.cai_model.v_net.parameters(), 'lr': 5e-5},
